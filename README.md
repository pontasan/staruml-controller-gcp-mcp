# StarUML Controller MCP Server — GCP Architecture Diagram

An MCP (Model Context Protocol) server specialized for **GCP Architecture Diagrams**. Enables AI assistants like Claude to programmatically create and manage GCP architecture diagrams — GCP elements, users, zones, products, services, and paths in [StarUML](https://staruml.io/).

<p align="center">
  <img src="images/main.jpg" alt="GCP Architecture Diagram created with staruml-controller-gcp-mcp" width="800">
</p>

## Architecture

<p align="center">
  <img src="images/architecture.svg" alt="Architecture: Claude ↔ MCP (stdio) ↔ staruml-controller-gcp-mcp ↔ HTTP REST API ↔ StarUML" width="800">
</p>
...
```bash
git clone https://github.com/pontasan/staruml-controller-gcp-mcp.git
cd staruml-controller-gcp-mcp
npm install && npm run build
```
...
5. All GCP Architecture Diagram tools become available via MCP
...
```json
{
  "mcpServers": {
    "staruml-gcp": {
      "command": "node",
      "args": ["/absolute/path/to/staruml-controller-gcp-mcp/dist/index.js"],
...
```bash
claude mcp add staruml-gcp -e STARUML_PASSWORD=<password> -- node /absolute/path/to/staruml-controller-gcp-mcp/dist/index.js
```
...
### GCP Architecture Diagram Tools

The core of this server — full CRUD for every GCP Architecture Diagram resource.

| Resource | Tools |
|---|---|
| **Diagrams** | `gcp_list_diagrams`, `gcp_create_diagram`, `gcp_get_diagram`, `gcp_update_diagram`, `gcp_delete_diagram` |
| **Elements** | `gcp_list_elements`, `gcp_create_element`, `gcp_get_element`, `gcp_update_element`, `gcp_delete_element` |
| **Paths** | `gcp_list_paths`, `gcp_create_path`, `gcp_get_path`, `gcp_update_path`, `gcp_delete_path` |
